...
#create the frequency table and convert counts to percentages
domain_counts <- table(taxonomy$domain)
nrow(taxonomy) - domain_counts["Unclassified"]
#152126
domain_percentages <- prop.table(domain_counts) * 100

#create the frequency table and convert counts to percentages
phylum_counts <- table(taxonomy$phylum)
nrow(taxonomy) - phylum_counts["Unclassified"]
#112594
phylum_percentages <- prop.table(phylum_counts) * 100

#create the frequency table and convert counts to percentages
class_counts <- table(taxonomy$class)
nrow(taxonomy) - class_counts["Unclassified"]
#71199
class_percentages <- prop.table(class_counts) * 100

#create the frequency table and convert counts to percentages
family_counts <- table(taxonomy$family)
nrow(taxonomy) - family_counts["Unclassified"]
#40629
family_percentages <- prop.table(family_counts) * 100

#N.B. kaiju was run on all contigs. CAT only on contig length >= 1000

#add contig information to taxonomy
taxonomy_contig <- merge(tpm_contig, taxonomy, by = "contig", all = FALSE)
...
#plot
svglite(filename="microbial_barplots_SI.svg", width=6, height=18, fix_text_size=FALSE)
...
svglite(filename="microbial_barplots_SI_no_legend.svg", width=4, height=18, fix_text_size=FALSE)
grid.arrange(p_boxplot_phylum_dataset, p_boxplot_class_dataset, 
             p_boxplot_family_dataset, nrow = 3)
dev.off()
...
maaslin_phy <- Maaslin2(
  ...
  output = "maaslin_phy_m", #output folder
...
#significant taxa
res_phy <- read.table("maaslin_phy_m/significant_results.tsv", header = TRUE, sep = "\t")
res_phy

#intersect with taxa in plots
intersect(res_phy[res_phy$metadata == "vb",]$feature, unique(phylum_contig_long_dt_top$phylum))
#[1] "Acidobacteriota" "Actinomycetota"  "Deinococcota"    "Basidiomycota"   "Chloroflexota"  
#[6] "Armatimonadota"  "Chlorophyta"     "Chytridiomycota" "Ascomycota" 
...
maaslin_cla <- Maaslin2(
  ...
  input_metadata = metadata,
  output = "maaslin_cla_m", #output folder
  fixed_effects = c("vb", "glacier"), #test both variables
  normalization = "NONE", #none because already TPM
  transform = "NONE", #none because already square root
  analysis_method = "LM", #linear model
...
#significant taxa
res_cla <- read.table("maaslin_cla_m/significant_results.tsv", header = TRUE, sep = "\t")
res_cla

#intersect with taxa in plots
intersect(res_cla[res_cla$metadata == "vb",]$feature, unique(class_contig_long_dt_top$class))
#[1] "Actinomycetes"       "Alphaproteobacteria" "Deinococci"          "Terriglobia"        
#[5] "Leotiomycetes"       "Microbotryomycetes"  "Chytridiomycetes" 
...
maaslin_fam <- Maaslin2(
  ...
  input_metadata = metadata,
  output = "maaslin_fam_m", #output folder
  fixed_effects = c("vb", "glacier"), #test both variables
  normalization = "NONE", #none because already TPM
  transform = "NONE", #none because already square root
  analysis_method = "LM", #linear model
...
#significant taxa
res_fam <- read.table("maaslin_fam_m/significant_results.tsv", header = TRUE, sep = "\t")
res_fam

#intersect with taxa in plots
intersect(res_fam[res_fam$metadata == "vb",]$feature, unique(family_contig_long_dt_top$family))
#[1] "Microbacteriaceae" "Deinococcaceae"    "Acidobacteriaceae" "Burkholderiaceae" 
#[5] "Cytophagaceae" 