...
output_file = "./assembly/cat_taxonomy_only_official_formatted_trimmed.csv"
	...
	for line in in_file:
		...
		if not line.startswith("#"):
			...
			if len(info) < 12: #contig not taxonomically assigned: "ORF has no hit to database" or "no taxid found"
				...
				orf = info[2].split(" ")[2]
				orf1 = orf.split("/")[0] #number of ORFs in a contig that were assigned to a taxonomy
				orf2 = orf.split("/")[1] #number of ORFs in a contig
				orf_ratio = float(orf1) / float(orf2)
				contig = info[0]
				if float(orf1) > 2 and float(orf_ratio) > 0.5: #keeping taxonomy only if more than two and more than half of the ORFs were assigned
					domain_t = info[5]
					phylum_t = info[6]
					class_t = info[7]
					order_t = info[8]
